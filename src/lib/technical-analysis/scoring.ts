...
function calcTrendIndicators(data: KlineData[]): IndicatorResult[] {
  ...
  if (emaArrays[9] && emaArrays[21] && n >= 3) {
    ...
    results.push({
      name: "EMA 9/21 Cross", value: currDiff,
      signal: crossSig, weight: 0.22, description: crossDesc,
    });
  }
  ...
  if (fanVals.length >= 3) {
    ...
    results.push({
      name: "MA Fan Order", value: fanSig, signal: fanSig, weight: 0.16,
  ...
  if (n >= 30) {
    ...
    if (isFinite(adxV)) {
      ...
      results.push({
        name: "ADX(14)", value: adxV, signal: adxSig, weight: 0.28,
        description:
  ...
  if (n >= 52) {
    ...
    if (isFinite(tkV) && isFinite(kjV) && isFinite(saV) && isFinite(sbV)) {
      ...
      const pos      = price > cloudTop ? "above cloud" : price < cloudBot ? "below cloud" : "inside cloud";
      results.push({
        name: "Ichimoku Cloud", value: price - cloudTop, signal: ichiSig, weight: 0.26,
  ...
  if (n >= 20) {
    ...
    const stSig = clip(dirV * 0.72, -1, 1);
    results.push({
      name: "SuperTrend", value: stV, signal: stSig, weight: 0.16,
  ...
  if (n >= 10) {
    ...
    results.push({
      name: "Heikin-Ashi", value: consecutive, signal: clip(haSig, -1, 1), weight: 0.20,
...
// ═══════════════════════════════════════════════
// MOMENTUM GROUP  (weight 0.25 base)
// ═══════════════════════════════════════════════
...
function calcMomentumIndicators(data: KlineData[]): IndicatorResult[] {
  ...
  if (isFinite(rsiV)) {
    ...
    results.push({
      name: "RSI(14)", value: rsiV, signal: rsiSig, weight: 0.28,
      description:
  ...
  if (n >= 35) {
    ...
    if (isFinite(mV) && isFinite(sV)) {
      ...
      results.push({
        name: "MACD", value: mV, signal: macdSig, weight: 0.32,
  ...
  if (n >= 17) {
    ...
    if (isFinite(kV) && isFinite(dV)) {
      ...
      results.push({
        name: "Stochastic(14,3)", value: kV, signal: clip(stochSig, -1, 1), weight: 0.22,
...
function calcVolumeIndicators(data: KlineData[]): IndicatorResult[] {
  ...
  if (n >= 20) {
    ...
    results.push({
      name: "OBV", value: obvV, signal: clip(obvSig, -1, 1), weight: 0.30,
  ...
  if (n >= 20) {
    ...
    results.push({
      name: "VWAP Rolling(20)", value: vwapR.value,
      signal: vwapR.signal, weight: 0.35,
  ...
  if (n >= 20) {
    ...
    results.push({
      name: "Volume Pressure", value: volRatio, signal: clip(volPSig, -1, 1), weight: 0.35,
...
// ═══════════════════════════════════════════════
// OSCILLATOR GROUP  (weight 0.15 base)
// ═══════════════════════════════════════════════
...
function calcOscillatorIndicators(data: KlineData[]): IndicatorResult[] {
  ...
  if (n >= 25) {
    ...
    if (isFinite(cciV)) {
      ...
      results.push({
        name: "CCI(20)", value: cciV, signal: clip(cciSig, -1, 1), weight: 0.28,
  ...
  if (n >= 14) {
    ...
    if (isFinite(wrV)) {
      ...
      results.push({
        name: "Williams %R(14)", value: wrV, signal: clip(wrSig, -1, 1), weight: 0.24,
  ...
  if (n >= 15) {
    ...
    if (isFinite(mfiV)) {
      ...
      results.push({
        name: "MFI(14)", value: mfiV, signal: mfiSig, weight: 0.24,
  ...
  if (n >= 20) {
    ...
    if (isFinite(ubV) && isFinite(lbV) && ubV !== lbV) {
      ...
      results.push({
        name: "Bollinger Bands(20)", value: pctB, signal: clip(bbSig, -1, 1), weight: 0.24,
...
// ═══════════════════════════════════════════════
// PATTERN GROUP  (weight 0.10 base)
// ═══════════════════════════════════════════════
...
function calcPatternIndicators(data: KlineData[]): IndicatorResult[] {
  ...
  // Doji
  if (body / range < 0.08) patterns.push(["Doji", 0.0]);
  ...
  if (n >= 3) {
    const threeUp   = [1, 2, 3].every(i => c[n - i] > o[n - i]) && c[n-1] > c[n-2] && c[n-2] > c[n-3];
    const threeDown = [1, 2, 3].every(i => c[n - i] < o[n - i]) && c[n-1] < c[n-2] && c[n-2] < c[n-3];
  ...
  if (patterns.length > 0) {
    ...
    const names    = patterns.map(([nm]) => nm).join(", ");
    results.push({
      name: "Candlestick Patterns", value: patterns.length,
      signal: clip(avgSig, -1, 1), weight: 0.50,
      description: names,
    });
  }
  ...
  if (n >= 50) {
    ...
    results.push({
      name: "Support/Resistance", value: price,
      signal: clip(srSig, -1, 1), weight: 0.50,
      description:
...
/**
 * Weighted average of a group of indicators (normalized)
...
/**
 * Calculate composite score for one timeframe
 * Returns compositeScore in [-100, +100]
 */
export function calculateTimeframeScore(
  ...
  // ── Collect all indicators by group ──
...
*/
  ...
  // ── Flatten all indicators for reporting ──
